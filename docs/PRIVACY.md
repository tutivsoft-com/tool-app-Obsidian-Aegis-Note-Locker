...
Aegis Note Locker is local-only. It does not require an account, collect analytics, call an AI service, upload vault content, or send passwords anywhere. Passwords, decrypted content, and encryption keys are not placed in notices, logs, clipboard data, plugin settings, or network requests.
